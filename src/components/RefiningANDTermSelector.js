...
if (!edges.hasOwnProperty("components")) {
  edges.components = {};
}
...
edges.components.RefiningANDTermSelector = class extends edges.Component {
  constructor(params) {
    ...
    this.deactivateThreshold = edges.util.getParam(
      params,
      "deactivateThreshold",
      false
    );
    ...
    this.ignoreEmptyString = edges.util.getParam(
      params,
      "ignoreEmptyString",
      true
    );
    ...
    this.excludePreDefinedFilters = edges.util.getParam(
      params,
      "excludePreDefinedFilters",
      true
    );
  ...
  synchronise() {
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      if (val != translate_val) {
        this.filters.push({ display: translate_val, term: val });
      } else {
        this.filters.push({ display: val, term: val });
      }
    }
  }
  ...
  _readValues(params) {
    ...
    if (this.excludePreDefinedFilters && this.edge.baseQuery) {
      predefined = this.edge.baseQuery.listMust(
        new es.TermFilter({ field: this.field })
      );
    }
  ...
  listAll() {
    ...
    bq.addAggregation(new es.TermsAggregation(params));
