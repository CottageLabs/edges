...
es.Aggregation = class {
  ...
  _parse_wrapper(obj, type) {
    ...
    var aggs = obj[this.name].aggs
      ? obj[this.name].aggs
      : obj[this.name].aggregations;
    if (aggs) {
...
es.Sort = class {
  constructor(params) {
    this.field = params.field || "score";
    this.order = params.order || "desc"; // Solr uses 'score' instead of '_score' for default relevance sorting
  ...
  objectify() {
    return `${this.field} ${this.order}`; // Solr uses 'field order' format as a string
  }

  parse(str) {
    const parts = str.split(" ");
    this.field = parts[0];
    this.order = parts[1] || "desc"; // Default to 'desc' if no order is provided
  }
};
...
es.Query = class {
  ...
  removeSortBy(params) {
    ...
    this.sort = this.sort.filter(
      (existingSort) => existingSort.field !== sort.field
    );
  }
  ...
  addMust(filter) {
    if (
      !this.listMust().some((existingFilter) => {
        return Object.keys(filter).every(
          (key) => existingFilter[key] === filter[key]
        );
      })
    ) {
  ...
  addMustNot(filter) {
    if (
      !this.listMustNot().some((existingFilter) => {
        return Object.keys(filter).every(
          (key) => existingFilter[key] === filter[key]
        );
      })
  ...
  addShould(filter) {
    if (
      !this.listShould().some((existingFilter) => {
        return Object.keys(filter).every(
          (key) => existingFilter[key] === filter[key]
        );
      })
  ...
  hasFilters() {
    return (
      this.must.length > 0 || this.should.length > 0 || this.mustNot.length > 0
    );
  }
  ...
  merge(source) {
    ...
    if (source.source) {
      this.addSourceFilters({
        include: source.getSourceIncludes(),
        exclude: source.getSourceExcludes(),
      });
    }
  }
  ...
  objectify(params) {
    ...
    if (
      Object.keys(query_part).length === 0 &&
...
es.QueryString = class {
  ...
  _unReplaceAll(string, find) {
    return string.replace(
      new RegExp("\\\\" + this._escapeRegExp(find), "g"),
      find
    );
  }
  ...
  _paired(string, pair) {
    const matches =
      string.match(new RegExp(this._escapeRegExp(pair), "g")) || [];
...
es.Result = class {
  ...
  buckets(facet_name) {
    if (this.data.facet_counts) {
      if (
        this.data.facet_counts.facet_fields &&
        this.data.facet_counts.facet_fields[facet_name]
      ) {
        return this._convertFacetToBuckets(
          this.data.facet_counts.facet_fields[facet_name]
        );
      } else if (
        this.data.facet_counts.facet_queries &&
        this.data.facet_counts.facet_queries[facet_name]
      ) {
        return this._convertFacetToBuckets(
          this.data.facet_counts.facet_queries[facet_name]
        );
...
function _es2solr({ query }) {
  ...
  if (Object.entries(query.query).length > 0) {
    ...
    if (queryPart.match) {
      ...
      solrQuery.q =
        rangeQueries.length > 1
          ? `(${rangeQueries.join(" OR ")})`
          : rangeQueries[0];
