...
if (!edges.hasOwnProperty("util")) {
  edges.util = {};
}
if (!edges.hasOwnProperty("es")) {
  edges.es = {};
}
...
edges.Edge = class {
  constructor(params) {
    ...
    this.preflightQueries = edges.util.getParam(
      params,
      "preflightQueries",
      false
    );
    ...
    this.openingQuery = edges.util.getParam(params, "openingQuery", () =>
      typeof es !== "undefined" ? new es.Query() : false
    );
    ...
    this.secondaryQueries = edges.util.getParam(
      params,
      "secondaryQueries",
      false
    );
    ...
    this.urlQuerySource = edges.util.getParam(
      params,
      "urlQuerySource",
      "source"
    );
    ...
    this.urlQueryOptions = edges.util.getParam(
      params,
      "urlQueryOptions",
      false
    );
    ...
    // the query adapter
    this.queryAdapter = edges.util.getParam(
      params,
      "queryAdapter",
      () => new edges.es.ESQueryAdapter()
    );
  ...
  runPreflightQueries(callback) {
    if (
      !this.preflightQueries ||
      Object.keys(this.preflightQueries).length === 0
    ) {
      callback();
      return;
    }
  ...
  runSecondaryQueries(callback) {
    ...
    if (
      ...
      Object.keys(this.secondaryQueries).length === 0
    ) {
      callback();
      return;
    }
    ...
    for (var key in this.secondaryQueries) {
      ...
      if (
        this.secondaryUrls !== false &&
        this.secondaryUrls.hasOwnProperty(key)
      ) {
...
edges.on = function (
  ...
  var clos = edges.util.eventClosure(
    caller,
    targetFunction,
    conditional,
    preventDefault
...
edges.util.delayer = function (fn, delay, timeout) {
  ...
  return function (event) {
    ...
    if (!timeout || !wait) {
      wait = setTimeout(throttler, delay);
    }
  };
};
...
edges.util.AsyncGroup = class {
  constructor(params) {
    ...
    this.successCallbackArgs = edges.util.getParam(
      params,
      "successCallbackArgs"
    );
  ...
  process(params) {
    ...
    for (let i = 0; i < this.list.length; i++) {
      ...
      let success_callback = edges.util.objClosure(
        this,
        "_actionSuccess",
        this.successCallbackArgs,
        context
      );
      let error_callback = edges.util.objClosure(
        this,
        "_actionError",
        this.successCallbackArgs,
        context
...
edges.util.cssClass = function (namespace, field, instance_name) {
  instance_name = edges.util._normaliseInstanceName(instance_name);
  let cls = namespace;
  if (instance_name) {
    cls += "_" + instance_name;
...
edges.util.numFormat = function (params) {
  var reflectNonNumbers = edges.util.getParam(
    params,
    "reflectNonNumbers",
    false
  );
  ...
  var thousandsSeparator = edges.util.getParam(
    params,
    "thousandsSeparator",
    false
  );
