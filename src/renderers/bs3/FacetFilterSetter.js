...
edges.renderers.bs3.FacetFilterSetter = class extends edges.Renderer {
    constructor(params) {
        super(params);
    ...
    draw() {
        ...
        for (var i = 0; i < comp.filters.length; i++) {
            ...
            if (active) {
                filters += '<strong>' + edges.util.escapeHtml(display);
                if (this.showCount) {
                    filters += " (" + count + ")";
                }
                ...
                filters += '<a href="#" class="' + valClass + '" data-filter="' + edges.util.escapeHtml(id) + '">' + edges.util.escapeHtml(display) + "</a>";
                if (this.showCount) {
