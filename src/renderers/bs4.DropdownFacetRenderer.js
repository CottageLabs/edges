$.extend(true, edges, {
    bs4 : {
        newDropdownFacetRenderer: function (params) {
            return edges.instantiate(edges.bs4.DropdownFacetRenderer, params, edges.newRenderer);
        },
        DropdownFacetRenderer: function (params) {
            ...
            this.namespace = "edges-bs4-dropdown-facet";
            ...
            this.draw = function () {
                ...
                if (values && values.length > 0) {
                    results = '<h6 class="dropdown-header">' + ts.display + '</h6>';

                    if (filterTerms.length > 0) {
                        results += '<a href="#" class="dropdown-item ' + removeClass + '">Clear filter</a>';
                    }
                        ...
                        if ($.inArray(val.term.toString(), filterTerms) === -1) {   // the toString() helps us normalise other values, such as integers
                            ...
                            if ((count === 0 && this.displayEmptyTerms) || count > 0) {
                                ...
                                if (first && filterTerms.length > 0) {
                                    ...
                                    results += '<h6 class="dropdown-header">' + filterHeader + '</h6>';
                                    first = false;
                                }
                                results += '<a href="#" class="dropdown-item ' + valClass + '" data-key="' + edges.escapeHtml(val.term) + '">' +
                                    edges.escapeHtml(val.display) + countFrag + "</a>";
                            }
                        }
                    }
                }

                var resultFrag = '<div class="dropdown-menu" aria-labelledby="' + buttonId + '"> \
                    ' + results + '\
                </div>';
                ...
                var buttonFrag = '<button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" id="' + buttonId + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">\
                    ' + filterFrag + ' \
                </button>';
